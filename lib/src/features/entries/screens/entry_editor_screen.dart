...
class _EntryEditorScreenState extends ConsumerState<EntryEditorScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      final currentState = ref.read(entryEditorProvider((entryId: widget.entryId, themeName: widget.themeName)));
      if (currentState.maybeWhen(
        success: (entry) => true,
        preview: (entry) => true,
        orElse: () => false,
      )) {
        ref.read(entryEditorProvider((entryId: widget.entryId, themeName: widget.themeName)).notifier).reset();
      }
    });
  }
  ...
  void _onContentChanged() {
    ...
    final markdownContent = MarkdownConverter.documentToMarkdown(_quillController.document);
    ref.read(entryEditorProvider((entryId: widget.entryId, themeName: widget.themeName)).notifier).updateContent(markdownContent);
  }
  ...
  void _showInlinePreview() {
    showDialog(
      ...
      ref.read(entryEditorProvider((entryId: widget.entryId, themeName: widget.themeName)).notifier).resetFromPreview();
    });
  }
    ...
    ref.listen<EntryEditorState>(entryEditorProvider((entryId: widget.entryId, themeName: widget.themeName)), (previous, next) {
      next.when(
        ...
        editing: (title, content, tags, privacy, isCommentable, isVotable, inLive, isShared, isDraft, images, entryId, hasUnsavedChanges, themeName, isAnonymous) {
          ...
          final currentContent = MarkdownConverter.documentToMarkdown(_quillController.document);
          if (currentContent != content) {
            // Convert markdown content back to Quill document
            _quillController.document = MarkdownConverter.markdownToDocument(content);
          }
        },
...
class _DraftPreviewDialog extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final l10n = AppLocalizations.of(context);
    final entryState = ref.watch(entryEditorProvider((entryId: entryId, themeName: themeName)));
    
    return entryState.when(
      initial: () => const Center(child: CircularProgressIndicator()),
      loading: () => const Center(child: CircularProgressIndicator()),
      editing: (title, content, tags, privacy, isCommentable, isVotable, inLive, isShared, isDraft, images, entryId, hasUnsavedChanges, themeName, isAnonymous) => 
        _buildPreviewContent(context, l10n, title, content, tags, privacy, isCommentable, isVotable, inLive, isShared, isAnonymous),
      publishing: (isUploadingImages, uploadProgress) => const Center(child: CircularProgressIndicator()),
      success: (entry) => const Center(child: CircularProgressIndicator()),
                      ...
                      children: [
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              Text(
                                isAnonymous ? (l10n?.anonymous ?? 'Anonymous') : 'You',
                    ...
                    Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 8),
                          _buildSettingRow(l10n?.privacy ?? 'Privacy', _getPrivacyText(privacy, l10n)),
                          _buildSettingRow(l10n?.comments ?? 'Comments', isCommentable ? (l10n?.enabled ?? 'Enabled') : (l10n?.disabled ?? 'Disabled')),
                          _buildSettingRow(l10n?.liveFeed ?? 'Live Feed', inLive ? (l10n?.enabled ?? 'Enabled') : (l10n?.disabled ?? 'Disabled')),
                          _buildSettingRow(l10n?.sharing ?? 'Sharing', isShared ? (l10n?.enabled ?? 'Enabled') : (l10n?.disabled ?? 'Disabled')),
                          if (isAnonymous) _buildSettingRow(l10n?.anonymous ?? 'Anonymous', l10n?.enabled ?? 'Enabled'),
                        ],
                      ),
                    ),
                    const SizedBox(height: 16),
                    ...
                    Container(
                      ...
                      child: Column(
                        children: [
                          ...
                          Text(
                            l10n?.entryNotPublishedYet ?? 'This entry is not published yet',
                            style: TextStyle(
  ...
  Widget _buildPreviewFromEntry(BuildContext context, AppLocalizations? l10n, MwEntry entry) {
  ...
  Widget _buildErrorContent(BuildContext context, AppLocalizations? l10n, String message) {
    return Dialog(
      child: Padding(
