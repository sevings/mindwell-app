...
final commentFeedProvider =
    StateNotifierProvider.family<CommentFeedNotifier, CommentFeedState, String>(
      (ref, username) {
        final usersApi = ref.read(usersApiProvider);

        return CommentFeedNotifier(username: username, usersApi: usersApi);
      },
    );
...
class CommentFeedNotifier extends StateNotifier<CommentFeedState> {
  ...
  CommentFeedNotifier({required String username, required UsersApi usersApi})
    : _username = username,
      _usersApi = usersApi,
  ...
  Future<void> fetchInitialComments() async {
    ...
    try {
      ...
      state = CommentFeedState.error(message: _getErrorMessage(e));
    }
  }
  ...
  /// Update the rating of a specific comment in the current state.
  ///
  /// This method is used to update comment ratings after voting without
  /// refetching the entire feed.
  ///
  /// [commentId] - The ID of the comment to update
  /// [newRating] - The new rating data
  void updateCommentRating(int commentId, MwRating newRating) {
    final currentState = state.when(
      ...
      error: (message, comments) => comments != null
          ? (comments: comments, hasMore: false, isFetchingMore: false)
          : null,
      empty: () => null,
    );
    ...
    final updatedComments = currentState.comments.map((comment) {
      if (comment.id == commentId) {
        // Create a new comment with updated rating using the builder pattern
        return comment.rebuild((b) => b.rating.replace(newRating));
      }
      return comment;
    }).toList();
    ...
    _logger.info('Updated rating for comment $commentId');
  }
