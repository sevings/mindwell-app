  ...
  group('CommentCard', () {
    ...
    setUp(() {
      ...
      mockAuthor = $MwUser((b) => b
        ..id = 1
        ..name = 'TestUser'
        ..avatar = MwAvatar((b) => b..x92 = 'https://example.com/avatar.jpg').toBuilder());
      ...
      mockComment = MwComment((b) => b
        ...
        ..content = '<p>This is a test comment with <strong>HTML</strong> content.</p>'
    ...
    testWidgets('displays comment information correctly', (WidgetTester tester) async {
      ...
      expect(find.textContaining('This is a test comment with HTML content.'), findsOneWidget);
    ...
    testWidgets('displays comment without entry title', (WidgetTester tester) async {
    ...
    testWidgets('handles empty comment content', (WidgetTester tester) async {
      final commentWithEmptyContent = mockComment.rebuild((b) => b
        ..content = null
        ..editContent = null);

      await tester.pumpWidget(createTestWidget(comment: commentWithEmptyContent));

      // Check that "Comment deleted" message is displayed
      expect(find.text('Comment deleted'), findsOneWidget);
    });

    testWidgets('handles long comment content with truncation', (WidgetTester tester) async {
      final longContent = '<p>${'This is a very long comment content. ' * 20}</p>';
      final commentWithLongContent = mockComment.rebuild((b) => b..content = longContent);

      await tester.pumpWidget(createTestWidget(comment: commentWithLongContent));

      // Check that content is truncated (should not contain the full content)
      expect(find.textContaining('This is a very long comment content.'), findsOneWidget);
      expect(find.textContaining('This is a very long comment content. This is a very long comment content. This is a very long comment content. This is a very long comment content. This is a very long comment content. This is a very long comment content. This is a very long comment content. This is a very long comment content. This is a very long comment content. This is a very long comment content. This is a very long comment content. This is a very long comment content. This is a very long comment content. This is a very long comment content. This is a very long comment content. This is a very long comment content. This is a very long comment content. This is a very long comment content. This is a very long comment content. This is a very long comment content.'), findsNothing);
    });

    testWidgets('displays voting state correctly when isVoting is true', (WidgetTester tester) async {
      await tester.pumpWidget(createTestWidget(isVoting: true));

      // Check that voting buttons are disabled (no tap handlers)
    ...
    testWidgets('calls onUpvote when upvote button is tapped', (WidgetTester tester) async {
      ...
      final upvoteIcon = find.byIcon(Icons.thumb_up_outlined);
      await tester.tap(upvoteIcon);
      await tester.pump();
    ...
    testWidgets('calls onDownvote when downvote button is tapped', (WidgetTester tester) async {
      ...
      await tester.pumpWidget(createTestWidget(onDownvote: onDownvote));

      // Find and tap the downvote button
      final downvoteIcon = find.byIcon(Icons.thumb_down_outlined);
      await tester.tap(downvoteIcon);
      await tester.pump();

      expect(downvoteCalled, isTrue);
    });

    testWidgets('does not call vote callbacks when isVoting is true', (WidgetTester tester) async {
      bool upvoteCalled = false;
      bool downvoteCalled = false;
      void onUpvote() => upvoteCalled = true;
      void onDownvote() => downvoteCalled = true;
    ...
    testWidgets('has correct tap targets for navigation', (WidgetTester tester) async {
      await tester.pumpWidget(createTestWidget());
    ...
    testWidgets('handles author without avatar', (WidgetTester tester) async {
      ...
      final commentWithAuthorWithoutAvatar = mockComment.rebuild((b) => b..author = authorWithoutAvatar.toBuilder().build());

      await tester.pumpWidget(createTestWidget(comment: commentWithAuthorWithoutAvatar));
    ...
    testWidgets('handles author without name', (WidgetTester tester) async {
      ...
      final commentWithAuthorWithoutName = mockComment.rebuild((b) => b..author = authorWithoutName.toBuilder().build());

      await tester.pumpWidget(createTestWidget(comment: commentWithAuthorWithoutName));

      // Check that fallback name is displayed
      expect(find.text('Unknown'), findsOneWidget);
    });

    testWidgets('strips HTML tags from content snippet', (WidgetTester tester) async {
      final commentWithHtml = mockComment.rebuild((b) => b
        ..content = '<p>This is <strong>bold</strong> and <em>italic</em> text with <a href="#">link</a>.</p>');
      ...
      expect(find.textContaining('This is bold and italic text with link.'), findsOneWidget);
