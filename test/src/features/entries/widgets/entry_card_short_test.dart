...
void main() {
  group('EntryCardShort', () {
    ...
    setUp(() {
      ...
      mockEntry = MwEntry(
        (b) => b
          ...
          ..content =
              '<p>This is a test entry content with <strong>HTML</strong> tags.</p>'
    ...
    testWidgets('displays entry title and content', (
      ...
      expect(
        find.text('This is a test entry content with HTML tags.'),
        findsOneWidget,
      );
    });
    ...
    testWidgets('does not display tags in short format', (
      WidgetTester tester,
    ) async {
      await tester.pumpWidget(createTestWidget());
    ...
    testWidgets('shows pinned indicator when entry is pinned', (
      WidgetTester tester,
    ) async {
    ...
    testWidgets('respects maxContentLines parameter', (
      ...
      final longContentEntry = mockEntry.rebuild(
        (b) => b
          ..content =
              '<p>This is a very long content that should be truncated when maxContentLines is set to 1. It contains multiple sentences and should not be displayed in full.</p>',
      );

      await tester.pumpWidget(
        createTestWidget(entry: longContentEntry, maxContentLines: 1),
      );
    ...
    testWidgets('hides image when showImage is false', (
      WidgetTester tester,
    ) async {
      final entryWithImage = mockEntry.rebuild(
        (b) => b
          ..images = ListBuilder([
            MwImage(
              (b) => b
      ...
      await tester.pumpWidget(
        createTestWidget(entry: entryWithImage, showImage: false),
      );
    ...
    testWidgets('uses cutContent and cutTitle when available', (
      WidgetTester tester,
    ) async {
      final entryWithCutContent = mockEntry.rebuild(
        (b) => b
          ..title = 'Full Title'
          ..cutTitle = 'Cut Title'
          ..content = '<p>This is the full content with lots of details.</p>'
          ..cutContent = '<p>This is the cut content.</p>',
      );

      await tester.pumpWidget(createTestWidget(entry: entryWithCutContent));

      // Should show cut title and cut content instead of full versions
      expect(find.text('Cut Title'), findsOneWidget);
      expect(find.text('This is the cut content.'), findsOneWidget);
      expect(find.text('Full Title'), findsNothing);
      expect(
        find.text('This is the full content with lots of details.'),
        findsNothing,
      );
    });
