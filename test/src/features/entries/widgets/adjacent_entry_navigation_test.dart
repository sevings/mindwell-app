...
void main() {
  group('AdjacentEntryNavigation', () {
    testWidgets('should not display when adjacentEntries is null', (
      tester,
    ) async {
      await tester.pumpWidget(
        MaterialApp(
          home: Scaffold(body: AdjacentEntryNavigation(adjacentEntries: null)),
        ),
      );
    ...
    testWidgets('should not display when both older and newer are null', (
      tester,
    ) async {
      final adjacentEntries = MwAdjacentEntries(
        (b) => b
          ..older = null
          ..newer = null,
      );
      ...
      await tester.pumpWidget(
        MaterialApp(
          home: Scaffold(
            body: AdjacentEntryNavigation(adjacentEntries: adjacentEntries),
          ),
        ),
      );
    ...
    testWidgets('should display only previous entry when older is provided', (
      tester,
    ) async {
      final olderEntry = MwCalendarEntry(
        (b) => b
          ..id = 1
      ...
      expect(
        ...
        findsOneWidget,
      ); // Right arrow for older entry
    });

    testWidgets('should display only next entry when newer is provided', (
      tester,
    ) async {
      final newerEntry = MwCalendarEntry(
        (b) => b
          ..id = 2
      ...
      expect(
        ...
        findsOneWidget,
      ); // Left arrow for newer entry
    });

    testWidgets('should display both entries when both are provided', (
      tester,
    ) async {
      final olderEntry = MwCalendarEntry(
        (b) => b
    ...
    testWidgets('should handle entries with empty titles', (tester) async {
      final entryWithEmptyTitle = MwCalendarEntry(
        (b) => b
          ..id = 1
          ..title = ''
          ..createdAt = 1640995200.0,
    ...
    testWidgets('should handle entries without timestamps', (tester) async {
      final entryWithoutTimestamp = MwCalendarEntry(
        (b) => b
          ..id = 1
          ..title = 'Test Entry'
          ..createdAt = null,
    ...
    testWidgets('should call onPreviousTap when previous entry is tapped', (
      tester,
    ) async {
    ...
    testWidgets('should call onNextTap when next entry is tapped', (
      tester,
    ) async {
    ...
    testWidgets('should handle long entry titles with ellipsis', (
      tester,
    ) async {
      final entryWithLongTitle = MwCalendarEntry(
        (b) => b
          ..id = 1
          ..title =
              'This is a very long entry title that should be truncated with ellipsis to fit in the available space'
          ..createdAt = 1640995200.0,
      );
      ...
      expect(
        find.textContaining('This is a very long entry title'),
        findsOneWidget,
      );
    });
  });
